...
villcode,
villname,
sum(case when chk = 1 then 1 else 0 end)/sum(case when chk is not null then 1 else 0 end)*100 as percent
from
(select
...
where  FLOOR((TO_DAYS(NOW())-TO_DAYS(person.birth))/365.25) >59 and  ((person.dischargetype is null) or (person.dischargetype = '9'))  and SUBSTRING(house.villcode,7,2) <> '00' $wvill) as tmp_per
left join
(select
visit.pcucodeperson,
visit.pid,
...
where FLOOR((TO_DAYS(NOW())-TO_DAYS(person.birth))/365.25) >59 and  ((person.dischargetype is null) or (person.dischargetype = '9'))  and SUBSTRING(house.villcode,7,2) <> '00' and visit.symptoms like '%ตรวจสุขภาพผู้สูงอายุ%'
...
$(function () {
        $('#container').highcharts({
            ...
            title: {
                text: '<?php echo "ร้อยละผู้สูงอายุมีสุขภาพที่พึงประสงค์" ?>'
            },
            subtitle: {
                text: '<?= $hosp ?>'
            },
