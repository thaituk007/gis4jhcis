...
villcode,
villname,
sum(case when chk != 0 then 1 else 0 end)/sum(case when chk is not null then 1 else 0 end)*100 as percent
from (select
*,
...
$(function () {
        $('#container').highcharts({
            ...
            title: {
                text: '<?php echo "ร้อยละความครอบคลุมวันซีน ของเด็กอายุ 0 - 71 เดือน" ?>'
            },
            subtitle: {
                text: '<?= $hosp ?>'
            },
