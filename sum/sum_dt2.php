...
(select count(distinct p.pid) from house h inner join person p on p.pcucodeperson = h.pcucode and p.hcode = h.hcode INNER JOIN visitepi ve ON p.pcucodeperson = ve.pcucodeperson AND p.pid = ve.pid where h.villcode = house.villcode and ((p.dischargetype is null) or (p.dischargetype = '9')) and (p.typelive = '1' or p.typelive = '0' or p.typelive = '3') and ve.vaccinecode in ('dT2','dTs2') and ve.dateepi between '$str' and '$sto') as perepi
FROM
village
