    ...
    <body class="skin-blue" onLoad="adjustWindow()">
		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				$txt .= "<select name='ds' id='ds' class='form-control' onchange='getData();'>
				<option value='00'>ทุกโรค</option>";
	$sql = "SELECT dc.group506code,dc.group506name 
				FROM
				house AS h
				...
				GROUP BY dc.group506code
				ORDER BY dc.group506code";
	$result=mysql_query($sql,$link);
	while($row=mysql_fetch_array($result)) {
		$txt .= "<option value='$row[group506code]'>$row[group506name]</option>";
	}
	$txt .= "</select>";
				$txt .= "
                            	</div><!-- /.input group -->
				...
				$txt .= "<table><tr><td><input type='button' class='btn btn-success' value='ok' onclick='getData();'></td><td><div id='csv'></div></td><td><div id='sum'></div></td><td><div id='chart'></div></td></tr></table><div id='slidebar'></div>";	 
				echo $txt;
?>
                	</div> <!-- //pleft-->
				</section>
...
function getData(){
			...
			var url = "../genxml/genxml_epe0.php?ds="+ds+"&str="+str+"&village="+village;
		  ...
		  var dc = xmldata[i].getAttribute("dc");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+"<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hono+"<br>โรค "+dc+" ป่วย "+sick;
		   	html2 = pname+"<br>บ้านเลขที่ "+hono+" หมู่ " +vill + "<br>" + dc+" ป่วย "+sick;
...
function getXls(){
		...
		var ds = document.getElementById('ds').value;
		var str = document.getElementById('datestart').value;	
		var url = "../xls/xls_epe0.php?ds="+ds+"&str="+str+"&village="+village;
	window.open(url,'data','top=60,left=250,width=700,height=550');
}
function getchart(){
		var village = document.getElementById('village').value;
		var ds = document.getElementById('ds').value;
		var str = document.getElementById('datestart').value;		
		var url = "../chart/chart_epe0.php?ds="+ds+"&str="+str+"&village="+village;
	window.open(url,'data','top=60,left=250,width=900,height=500');
}
function getsum(){
		var village = document.getElementById('village').value;
		var ds = document.getElementById('ds').value;
		var str = document.getElementById('datestart').value;		
		var url = "../sum/sum_epe0.php?ds="+ds+"&str="+str+"&village="+village;
	window.open(url,'data','top=60,left=250,width=800,height=550');
}
</script>
