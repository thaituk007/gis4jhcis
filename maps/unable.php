    ...
    <body class="skin-blue" onLoad="adjustWindow()">
		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<form role="form">
        		<?php
				$txt = "<p align='center'><strong>ข้อมูลผู้พิการ</strong></p>";
				...
				$txt .= "<select name='utype' id='utype' class='form-control' onchange='getData();'>
				<option value='00'>ทุกประเภท</option>";
				$sql = "SELECT
				pin.incompletecode,pin.incompletename 
				FROM house AS h
				...
				GROUP BY pin.incompletecode";
				$result=mysql_query($sql,$link);
				while($row=mysql_fetch_array($result)) {
				$txt .= "<option value='$row[incompletecode]'>$row[incompletename]</option>";
				}
				$txt .= "</select>";
				$txt .= "</div><!-- /.input group -->
                		</div><!-- /.form group -->";
...
function getData(){
			...
			var url = "../genxml/genxml_unable.php?utype="+utype+"&village="+village;
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+"<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hono;
		   	html2 = pname+"<br>บ้านเลขที่ "+hono+" หมู่ " +vill + "<br>" + dsc;
