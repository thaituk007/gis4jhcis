    ...
    <body class="skin-blue" onLoad="adjustWindow()">
		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<form role="form">
        		<?php
				$txt = "<p align='center'><strong>ประชาชนหญิงที่ได้รับ<br>การตรวจคัดกรองมะเร็งปากมดลูก</strong></p>";
				...
				$txt .= "<div class='form-group'>
             				<label>เลือกกลุ่ม:</label>
                                ...
                                <select name='chk_stool' id='chk_stool' class='form-control' onchange='getData();'><option value='1'>แสดงทุกคน</option><option value='2'>เฉพาะผู้ที่ได้รับการตรวจ</option><option value='3'>เฉพาะผู้ที่ผลตรวจผิดปกติ</option><option value='4'>เฉพาะผู้ที่ไม่ได้ตรวจ</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";
				$txt .= "<div class='form-group'>
             				<label>กลุ่มอายุ:</label>
                                ...
                                <select name='getage' id='getage' class='form-control' onchange='getData();'><option value='1'>อายุ 30 - 60 ปี </option><option value='2'>อายุต่ำกว่า 30 ปี</option><option value='3'>อายุ 60 ปี ขึ้นไป</option><option value='4'>ทุกกลุ่มอายุ</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";
				$txt .= "<div class='form-group'>
             				<label>เลือกประเภทอยู่อาศัย:</label>
                  				<div class='input-group'>
                   					<div class='input-group-addon'>
                     				<i class='fa fa-calendar'></i>
                      				</div>
				...
				$txt .= "<table><tr><td><input type='button' class='btn btn-success' value='ok' onclick='getData();'></td><td><div id='csv'></div></td><td><div id='sum'></div></td><td><div id='chart'></div></td></tr></table><div id='gresult'></div><div id='dtl'></div><div id='slidebar'></div>";	 
				echo $txt;
?>
        		</form>
                	</div>
...
function getData(){
			...
			var chk_stool = document.getElementById('chk_stool').value;
			...
			var url = "../genxml/genxml_papsmear.php?str="+str+"&village="+village+"&chk_stool="+chk_stool+"&getage="+getage+"&live_type="+live_type;
		  ...
		  var resultmean = xmldata[i].getAttribute("resultmean");
		  var result = xmldata[i].getAttribute("result");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			if(sick == ""){sm = "ยังไม่ได้ตรวจ";}else{sm = "วันที่ตรวจ "+sick+" ผลการตรวจ  "+resultmean;}
			html = pname+" อายุ "+age+" ปี<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hono+"<br>"+sm;
		   	html2 = pname+" อายุ "+age+" ปี<br>บ้านเลขที่ "+hono+" หมู่ " +vill +"<br>"+sm;
			var icon;
			if(result == "0"){
				hno = hno + 1;
			 	icon = customIcons['c1'];
				...
				kok = kok + 1;
				icon = customIcons['c2'];
			}else if(result == "2" | result == "3" | result == "5" | result == "6" | result == "9" | result == "1"){
				rok = rok + 1;
				icon = customIcons['c0'];	
			}else{
				hok = hok + 1;
			...
			var legent = "<img src='../img/c002.png'> = ปกติ "+hno+" คน<br> <img src='../img/c001.png'> = ผิดปกติ "+rok+"คน<br> <img src='../img/c003.png'> = รอผล "+kok+" คน<br> <img src='../img/c006.png'> = ยังไม่ได้ตรวจ "+hok+" คน";		
		...
		document.getElementById("gresult").innerHTML = "<input type='button' value='สรุปรอบ5ปี' class='btn btn-warning' onclick='getresult();'>";		
}
...
function getXls(){
		...
		var getage = document.getElementById('getage').value;
		var live_type = document.getElementById('live_type').value;		
		var url = "../xls/xls_papsmear.php?str="+str+"&village="+village+"&chk_stool="+chk_stool+"&getage="+getage+"&live_type="+live_type;
...
function getchart(){
		...
		var getage = document.getElementById('getage').value;
		var live_type = document.getElementById('live_type').value;			
		var url = "../chart/chart_papsmear.php?str="+str+"&village="+village+"&getage="+getage+"&live_type="+live_type;
...
function getsum(){
		...
		var getage = document.getElementById('getage').value;
		var live_type = document.getElementById('live_type').value;			
		var url = "../sum/sum_papsmear.php?str="+str+"&village="+village+"&getage="+getage+"&live_type="+live_type;
	window.open(url,'data','top=120,left=250,width=800,height=550');
}
function getresult(){			
		var url = "../sum/sum_papsmeary5.php";
	window.open(url,'data','top=120,left=250,width=800,height=550');
}
</script>
