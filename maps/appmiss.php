			...
			<aside class="left-side sidebar-offcanvas">
				...
				$txt .= "<div class='form-group'>
             				<label>ประเภทการนัด : </label>
                                ...
                                <select name='app_type' id='app_type' class='form-control' onchange='getData();'><option value='chronic'>นัดผู้ป่วยโรคเรื้อรัง</option><option value='epi'>นัดวัคซีน</option><option value='anc'>นัดฝากครรภ์</option><option value='fp'>นัดวางแผนครอบครัว</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";
				$txt .= "<div class='form-group'>
				...
				$txt .= "<div class='form-group'>
             				<label>เลือกกลุ่ม : </label>
                                ...
                                <select name='chk_ncd' id='chk_ncd' class='form-control' onchange='getData();'><option value='1'>แสดงทุกคน</option><option value='2'>มาตามนัด</option><option value='3'>ไม่มาตามนัด</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";
...
function getData(){
			...
			var chk_ncd = document.getElementById("chk_ncd").value;		
			var url = "../genxml/genxml_appmiss.php?str="+str+"&village="+village+"&app_type="+app_type+"&chk_ncd="+chk_ncd;
		  ...
		  var hno = xmldata[i].getAttribute("hno");
		  var moo = xmldata[i].getAttribute("moo");
		  var appsick = xmldata[i].getAttribute("appsick");
		  var apptypename = xmldata[i].getAttribute("apptypename");
		  var sicksign = xmldata[i].getAttribute("sicksign");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+"<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hno+" <br>วันนัด "+appsick+"<br>นัด"+apptypename+" "+sicksign;
		   	html2 = pname+"<br>บ้านเลขที่ "+hno+" หมู่ " +moo+" <br>วันนัด "+appsick + "<br>นัด " + apptypename+" "+sicksign;
			var icon;
			if(sicksign == "ขาดนัด"){
				rok = rok + 1;
			 	icon = customIcons['c0'];	
			...
			var legent = "<img src='../img/c002.png'> = มาตามนัด "+hok+" คน<br> <img src='../img/c001.png'> = ขาดนัด "+rok+" คน";		
...
function getXls(){
	...
	var app_type = document.getElementById("app_type").value;
	var str = document.getElementById('datepicker-th1').value;
	var url = "../xls/xls_appmiss.php?str="+str+"&village="+village+"&app_type="+app_type;
...
function getchart(){
	...
	var app_type = document.getElementById("app_type").value;
	var str = document.getElementById('datepicker-th1').value;
	var url = "../chart/chart_appmiss.php?str="+str+"&village="+village+"&app_type="+app_type;
	window.open(url,'data','top=120,left=250,width=800,height=500');
}
function getsum(){
	var village = document.getElementById('village').value;
	var app_type = document.getElementById("app_type").value;
	var str = document.getElementById('datepicker-th1').value;
	var url = "../sum/sum_appmiss.php?str="+str+"&village="+village+"&app_type="+app_type;
	window.open(url,'data','top=120,left=250,width=800,height=500');
}
</script>
