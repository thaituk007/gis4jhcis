		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<section class="sidebar">
                	<div id="pleft">
        		<?php
				$txt = "<p align='center'><strong>ผู้ป่วยวัณโรคที่ได้รับการเยี่ยมบ้าน</strong></p>";
				...
				$txt .= "<div class='form-group'>
             				<label>กลุ่ม</label>
                  				<div class='input-group'>
                                	...
                                	<select name='chk_stool' id='chk_stool' class='form-control' onchange='getData();'><option value='1'>แสดงทุกคน</option><option value='2'>เฉพาะผู้ที่ได้รับการเยี่ยม</option><option value='3'>เฉพาะผู้ที่ไม่ได้เยี่ยม</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";	
...
function getData(){
			...
			var url = "../genxml/genxml_home_visit_tb.php?str="+str+"&village="+village+"&chk_stool="+chk_stool;
		  ...
		  var pid = xmldata[i].getAttribute("pid");
		  ...
		  var count_visit = xmldata[i].getAttribute("count_visit");
		  var chronic_code = xmldata[i].getAttribute("chronic_code");
		  var chronic_name = xmldata[i].getAttribute("chronic_name");
		  var homehealthdetail = xmldata[i].getAttribute("homehealthdetail");
		  var sick = xmldata[i].getAttribute("sick");
		  var chk = xmldata[i].getAttribute("chk");
		  var userh = xmldata[i].getAttribute("userh");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+" อายุ "+age+" ปี<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hono+"<br>โรคเรื้อรัง "+chronic_name+"<br>"+chk+" "+count_visit+" ครั้ง เยี่ยมล่าสุด  "+sick;
		   	html2 = pname+" อายุ "+age+" ปี<br>บ้านเลขที่ "+hono+" หมู่ " +vill +"<br>โรคเรื้อรัง "+chronic_name+"<br>"+chk+" "+count_visit+" ครั้ง เยี่ยมล่าสุด "+sick+"<br> โดย "+userh+" <br><a href='#' onclick='hom_tb("+pid+");'>รายละเอียด</a>";
			var icon;
			if(count_visit > 0){
				hno = hno + 1;
			 	icon = customIcons['c0'];	
			...
			var legent = "<img src='../img/c001.png'> = ได้รับการเยี่ยม "+hno+" <br> <img src='../img/c006.png'> = ไม่ได้เยี่ยม "+hok;		
...
function getXls(){
		...
		var str = document.getElementById('datestart').value;
		var chk_stool = document.getElementById('chk_stool').value;			
		var url = "../xls/xls_home_visit_tb.php?str="+str+"&village="+village+"&chk_stool="+chk_stool;
	window.open(url,'data','top=120,left=250,width=700,height=550');
}
function getchart(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datestart').value;		
		var url = "../chart/chart_home_visit_tb.php?str="+str+"&village="+village;
	window.open(url,'data','top=120,left=250,width=900,height=500');
}
function getsum(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datestart').value;		
		var url = "../sum/sum_home_visit_tb.php?str="+str+"&village="+village;
	window.open(url,'data','top=120,left=250,width=800,height=550');
}
function hom_tb(fpid){
	var str = document.getElementById('datestart').value;			
	var tget = "../xls/xls_home_visit_tb_list.php?str="+str+"&pid="+fpid;
	window.open(tget,'Upfile','top=150,left=300,width=650,height=400');	
}
</script>
