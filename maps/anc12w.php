    ...
    <body class="skin-blue" onLoad="adjustWindow()">
		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<form role="form">
        		<?php
				$txt = "<p align='center'><strong>รายงานหญิงตั้งครรภ์<br>ฝากครรภ์ครั้งแรกก่อน 12 สัปดาห์</strong></p>";
...
function getData(){
			...
			var url = "../genxml/genxml_anc_12_w.php?str="+str+"&village="+village;
		  ...
		  var lmp = xmldata[i].getAttribute("lmp");
		  var edc = xmldata[i].getAttribute("edc");
		  var first_visit_date = xmldata[i].getAttribute("first_visit_date");
		  var pregno = xmldata[i].getAttribute("pregno");
		  var agepreg = xmldata[i].getAttribute("agepreg");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+" อายุ "+age+" ปี<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ "+hono+"<br> ฝากครรภ์ครั้งแรก "+first_visit_date+" ตอนอายุครรภ์ "+agepreg+" สัปดาห์";
		   	html2 = pname+" อายุ "+age+" ปี<br>บ้านเลขที่ "+hono+" หมู่ " +vill +"<br> ฝากครรภ์ครั้งแรก"+first_visit_date+" ตอนอายุครรภ์ "+agepreg+" สัปดาห์";
			var icon;
			if(chk == "1"){
				hno = hno + 1;
			 	icon = customIcons['c0'];
			...
			var legent = "<img src='../img/c001.png'> = ฝากครรภ์ก่อน 12 week "+hno+" คน<br> <img src='../img/c003.png'> = ฝากครรภ์หลัง 11 week "+rok+" คน<br> <img src='../img/c006.png'> = ไม่ระบุ "+hok+" คน";		
...
google.maps.event.addDomListener(window, 'load', initialize); 
	  	$(function(){
		  $("#datepicker-th1").datepicker({ format: "yyyy-mm-dd", language: "th", autoclose: true, todayHighlight: true, defaultDate: '<?php echo $daysdatepick; ?>'});
		  $("#datepicker-th2").datepicker({ format: "yyyy-mm-dd", language: "th", autoclose: true, todayHighlight: true, defaultDate: '<?php echo $daysdatepick; ?>'});
		});
...
function getXls(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datestart').value;		
		var url = "../xls/xls_anc_12_w.php?str="+str+"&village="+village;
	window.open(url,'data','top=120,left=250,width=700,height=550');
}
function getchart(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datestart').value;		
		var url = "../chart/chart_anc_12_w.php?str="+str+"&village="+village;
	window.open(url,'data','top=120,left=250,width=900,height=500');
}
function getsum(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datestart').value;		
		var url = "../sum/sum_anc_12_w.php?str="+str+"&village="+village;
