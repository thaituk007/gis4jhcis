		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<section class="sidebar">
                	<div id="pleft">
        		<?php
				$txt = "<p align='center'><strong>รายงานการตรวจพยาธิ</strong></p>";
				...
				$txt .= "<div class='form-group'>
             				<label>กลุ่ม : </label>
                  				<div class='input-group'>
                                	...
                                	<select name='chk_stool' id='chk_stool' class='form-control' onchange='getData();'><option value='1'>แสดงทุกคน</option><option value='2'>เฉพาะผู้ที่ได้รับการตรวจ</option><option value='3'>เฉพาะผู้ที่ตรวจพบพยาธิ</option><option value='4'>ตรวจพบพยาธใบไม้ตับ</option><option value='5'>เฉพาะผู้ที่ไม่ได้ตรวจ</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";
				$txt .= "<div class='form-group'>
             				<label>กลุ่มอายุ : </label>
                  				<div class='input-group'>
                                	...
                                	<select name='getage' id='getage' class='form-control' onchange='getData();'><option value='30'>อายุ 30 ปี ขึ้นไป</option><option value='20'>อายุต่ำกว่า 30 ปี</option><option value='40'>อายุ 40 ปี ขึ้นไป</option><option value='35'>อายุ 30 - 39 ปี</option><option value='99'>แสดงทุกคน</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";
				$txt .= "<div class='form-group'>
...
function getData(){
			...
			var url = "../genxml/genxml_stool_ov.php?str="+str+"&village="+village+"&chk_stool="+chk_stool+"&getage="+getage+"&live_type="+live_type;
		  ...
		  var para = xmldata[i].getAttribute("para");
		  var diseasenamethai = xmldata[i].getAttribute("diseasenamethai");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+" อายุ "+age+" ปี<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hono+"<br>"+sick+"<br>"+chk;
		   	html2 = pname+" อายุ "+age+" ปี<br>บ้านเลขที่ "+hono+" หมู่ " +vill +"<br>"+sick+"<br>"+chk;
			var icon;
			if(chk == "ยังไม่ได้ตรวจ"){
				rok = rok + 1;
			 	icon = customIcons['c1'];
			}else if(chk == "ผลการตรวจ ไม่พบ"){
				hno = hno + 1;
				icon = customIcons['c2'];	
			}else{
				hok = hok + 1;
			...
			var legent = "<img src='../img/c001.png'> = พบพยาธิ "+hok+" <br> <img src='../img/c003.png'> = ไม่พบพยาธิ "+hno+"<br> <img src='../img/c006.png'> = ไม่ได้ตรวจ "+rok;		
...
function getXls(){
		...
		var getage = document.getElementById('getage').value;
		var live_type = document.getElementById('live_type').value;		
		var url = "../xls/xls_stool_ov.php?str="+str+"&village="+village+"&chk_stool="+chk_stool+"&getage="+getage+"&live_type="+live_type;
...
function getchart(){
		...
		var getage = document.getElementById('getage').value;
		var live_type = document.getElementById('live_type').value;			
		var url = "../chart/chart_stool_ov.php?str="+str+"&village="+village+"&getage="+getage+"&live_type="+live_type;
...
function getsum(){
		...
		var getage = document.getElementById('getage').value;
		var live_type = document.getElementById('live_type').value;			
		var url = "../sum/sum_stool_ov.php?str="+str+"&village="+village+"&getage="+getage+"&live_type="+live_type;
