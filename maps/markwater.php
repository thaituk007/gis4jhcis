...
<form role="form">
        		<?php
				$txt = "<p align='center'><strong>บันทึก/แก้ไข<br> ตำแหน่งแหล่งน้ำสาธารณะ</strong></p><br><div class='text text-danger'> กรุณาเลือกหมู่บ้านก่อนบันทึก</div>";
...
var customIcons = {
  ...
  function getData(){
			...
			var url = "../genxml/genxml_markwater.php?villcode="+villcode;
		  ...
		  var mno = xmldata[i].getAttribute("mno");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = "<img src='../img/"+simg+"'> ลำดับที่ " + mno+"<br>"+hname+"<br>"+type;
		   	html2 = "ลำดับที่ "+mno+" หมู่ " +vill + "<br>" + hname+
						"<br>"+type+"<br><input type='button' value='ลบจุดบ้าน' onclick='delData("+hcode+");'/>";
		...
		var inshtml = "หมู่ที่ "+mu+" รหัสบ้าน"+slmooban+"<br>ลำดับที่ <input type='text' size='8' name='fsno' id='fsno'><br>"+
...
function delData(hcode){
  createXMLHttpRequest();
      var urld = "../execute/execute_water.php?hcode=" + hcode;	
...
function saveData(){
	  ...
	  var fsno = document.getElementById('fsno').value;
      var url = "../execute/execute_water.php?villagecode="+villagecode+"&fsno="+fsno+"&lat="+uplat+"&lng="+uplng; 
	  createXMLHttpRequest();
