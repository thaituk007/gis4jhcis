		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<section class="sidebar">
                	<div id="pleft">
        		<?php
				$txt = "<p align='center'><strong>ข้อมูลระบาดวิทยา</strong></p>";
             				...
             				<label>กลุ่มอายุ : </label>
                  				<div class='input-group'>
                     				...
                     				<i class='fa fa-tasks'></i>
										...
										<select name='getage' id='getage' class='form-control' onchange='getData();'><option value='15'>อายุ 15 ปี ขึ้นไป</option><option value='35'>อายุ 35 ปี ขึ้นไป</option><option value='34'>อายุ 15 - 34 ปี</option></select>
								</div><!-- /.input group -->
                		</div><!-- /.form group -->";
				$txt .= "<div class='form-group'>
             				<label>กลุ่มป่วย/ไม่ป่วย : </label>
                  				<div class='input-group'>
                   					<div class='input-group-addon'>
                     				<i class='fa fa-tasks'></i>
                      				</div>
										<select name='getchronic' id='getchronic' class='form-control' onchange='getData();'><option value='9'>แสดงทุกคน</option><option value='1'>เฉพาะคนที่ยังไม่ป่วย</option></select>
								</div><!-- /.input group -->
                		</div><!-- /.form group -->";
				$txt .= "<div class='form-group'>
             				<label>กลุ่มเสี่ยง : </label>
                  				<div class='input-group'>
                     				...
                     				<i class='fa fa-tasks'></i>
										...
										<select name='risk' id='risk' class='form-control' onchange='getData();'><option value='9'>แสดงทุกคน</option><option value='4'>เสี่ยงเบาหวานหรือความดัน</option><option value='1'>เสี่ยงเบาหวาน</option><option value='2'>เสี่ยงความดันโลหิตสูง</option><option value='3'>เสี่ยงเบาหวานและความดัน</option></select>
								</div><!-- /.input group -->
                		</div><!-- /.form group -->";			
...
function getData(){
			...
			var getchronic = document.getElementById('getchronic').value;			
			var url = "../genxml/genxml_ncd_screen.php?village="+village+"&str="+str+"&live_type="+live_type+"&getage="+getage+"&risk="+risk+"&getchronic="+getchronic;
		  ...
		  var dm = xmldata[i].getAttribute("dm");
		  var ht = xmldata[i].getAttribute("ht");
		  var waist = xmldata[i].getAttribute("waist");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+"<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hono+"<br>ผลความดัน "+ht+", ผลเบาหวาน "+dm+", "+waist+"<br>วันที่คัดกรอง "+sick;
		   	html2 = pname+"<br>บ้านเลขที่ "+hono+" หมู่ " +vill + "<br>ผลความดัน "+ht+", ผลเบาหวาน "+dm+", "+waist+"<br>วันที่คัดกรอง "+sick;
			var icon;
			if(chk == "2"){
				hok = hok + 1;
			 	icon = customIcons['c0'];	
			...
			var legent = "<img src='../img/c001.png'> = เสี่ยงเบาหวานหรือความดัน "+hok+" คน<br> <img src='../img/c004.png'> = ปกติ "+rok+" คน<br> <img src='../img/c006.png'> = ยังไม่ได้รับการคัดกรอง "+kok+" คน";		
...
function getXls(){
	...
	var risk = document.getElementById('risk').value;
	var getchronic = document.getElementById('getchronic').value;
	var url = "../xls/xls_ncd_screen.php?village="+village+"&str="+str+"&live_type="+live_type+"&getage="+getage+"&risk="+risk+"&getchronic="+getchronic;
...
function getchart(){
	...
	var risk = document.getElementById('risk').value;
	var getchronic = document.getElementById('getchronic').value;
	var url = "../chart/chart_ncd_screen.php?village="+village+"&str="+str+"&live_type="+live_type+"&getage="+getage+"&risk="+risk+"&getchronic="+getchronic;
...
function getsum(){
	...
	var risk = document.getElementById('risk').value;
	var getchronic = document.getElementById('getchronic').value;
	var url = "../sum/sum_ncd_screen.php?village="+village+"&str="+str+"&live_type="+live_type+"&getage="+getage+"&risk="+risk+"&getchronic="+getchronic;
	window.open(url,'data','top=120,left=250,width=800,height=500');
}
</script>
