		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<section class="sidebar">
                	<div id="pleft">
        		<?php
$txt = "<p align='center'><strong>ประชากรตามกลุ่มอายุ</strong></p>";
$txt .= "<div class='form-group'>
             				<label>อายุ : ( ปี (1-5,7,9))</label>
                  			<div class='input-group'>
                   				<div class='input-group-addon'>
				   				<i class='fa fa-tasks'></i>
				   				</div><input type='text' name='age' id='age' class='form-control' size='8' value='0-1'></div><!-- /.input group -->
                		</div><!-- /.form group -->";
$txt .= "<div class='form-group'>
             				<label>เพศ :</label>
                  			<div class='input-group'>
...
$txt .= "</select></div><!-- /.input group -->
             				...
             				<label>ประเภทการอยู่อาศัย:</label>
                  			<div class='input-group'>
				   				...
				   				</div><select name='live_type' id='live_type' class='form-control'  onchange='getData();'><option value='99'>ทั้งหมดในเขตรับผิดชอบ(0,1,2,3)</option><option value='1'>ที่อาศัยอยู่จริง (0,1,3)</option><option value='2'>ตามทะเบียนบ้าน(0,1,2)</option></select></div><!-- /.input group -->
                		</div><!-- /.form group -->
                                    <!-- Date dd/mm/yyyy -->
                                    <div class='form-group'>
                                        <label>วันที่อ้างอิงคำนวนอายุ</label>
                                        <div class='input-group'>
                                            <div class='input-group-addon'>
...
function getData(){
			...
			var datemask = document.getElementById('datepicker-th1').value;
			var age = document.getElementById('age').value;
			var sex = document.getElementById('sex').value;
			var village = document.getElementById('village').value;
			var live_type = document.getElementById('live_type').value;
			var url = "../genxml/genxml_person.php?age="+age+"&sex="+sex+"&village="+village+"&live_type="+live_type+"&datemask="+datemask;
		  ...
		  var dsc = xmldata[i].getAttribute("bod");
		  var ag = xmldata[i].getAttribute("ag");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+" ("+ag+" ปี)"+"<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hono;
		   	html2 = pname+"<br>บ้านเลขที่ "+hono+" หมู่ " +vill + "<br>" + dsc+" ("+ag+" ปี)";
...
function getXls(){
		...
		var village = document.getElementById('village').value;
		var live_type = document.getElementById('live_type').value;
		var url = "../xls/xls_person.php?age="+age+"&sex="+sex+"&village="+village+"&live_type="+live_type+"&datemask="+datemask;
...
function getchart(){
		...
		var village = document.getElementById('village').value;
		var live_type = document.getElementById('live_type').value;
		var url = "../chart/chart_person.php?age="+age+"&sex="+sex+"&village="+village+"&live_type="+live_type+"&datemask="+datemask;
...
function getsum(){
		...
		var village = document.getElementById('village').value;
		var live_type = document.getElementById('live_type').value;
		var url = "../sum/sum_person.php?age="+age+"&sex="+sex+"&village="+village+"&live_type="+live_type+"&datemask="+datemask;
	window.open(url,'data','top=120,left=250,width=800,height=550');
}
</script>
