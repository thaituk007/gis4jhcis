		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<section class="sidebar">
                	<div id="pleft">
        		<?php
				$txt = "<p align='center'><strong>รายงานเด็กอายุ 9 - 24 เดือน<br>ที่ได้รับการตรวจฟัน</strong></p>";
				...
				$txt .= "<div class='form-group'>
             				<label>ระหว่างวันที่:</label>
                               ...
                               <input name='strdate' class='form-control' type='text' id='datepicker-th1' onkeypress='date_fbb(this);' value='$cuday'/>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";		
...
$sql = "SELECT * FROM cchart_bh";
	$result=mysql_query($sql,$link);
	if($result){echo $txt;}else{echo "<p align='center'><strong>ไม่พบตารางอ้างอิงในการประมวลผล<br><a href='../sql/bigdump.php?start=1&fn=nutri.sql&foffset=0&totalqueries=0&delimiter=%3B'>โปรดคลิกที่นี่เพื่อนำเข้าตารางก่อนออกรายงาน</a></strong></p>";}
?>
                	</div> <!-- //pleft-->
				</section>
			</aside>
...
function getData(){
			...
			var str = document.getElementById('datepicker-th1').value;		
			var url = "../genxml/genxml_epi_tooth.php?str="+str+"&village="+village;
		  ...
		  var epi_chk = xmldata[i].getAttribute("epi_chk");
		  var chk = xmldata[i].getAttribute("chk");
		  var toothmilk = xmldata[i].getAttribute("toothmilk");
		  var toothmilkcorrupt = xmldata[i].getAttribute("toothmilkcorrupt");
		  var toothpermanent = xmldata[i].getAttribute("toothpermanent");
		  var toothpermanentcorrupt = xmldata[i].getAttribute("toothpermanentcorrupt");
		  var tartar = xmldata[i].getAttribute("tartar");
		  var gumstatus = xmldata[i].getAttribute("gumstatus");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+" อายุ "+age+" ปี<br><img src='../img/"+simg+"'>บ้านเลขที่ "+hono+" หมู่ " +vill +"<br>อายุ"+age+" เดือน "+epi_chk;
		   	html2 = pname+" อายุ "+age+" ปี<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ "+hono+"<br>อายุ "+age+" เดือน "+epi_chk+"<br>ฟันน้ำนม/ฟันผุ(ซี่) "+toothmilk+"/"+toothmilkcorrupt+" ฟันแท้/ฟันผุ(ซี่) "+toothpermanent+"/"+toothpermanentcorrupt+"<br>หินน้ำลาย "+tartar+" เหงือกอักเสบ "+gumstatus;
			var icon;
			if(chk == "1"){
				hno = hno + 1;
			 	icon = customIcons['c0'];
			...
			if(chk == "1"){if(toothmilkcorrupt > 0){
				hok = hok + 1;
				icon = customIcons['c2'];
			}else{
			}
			...
			var legent = "<img src='../img/c001.png'> = ได้รับการตรวจฟัน "+hno+" คน<br> <img src='../img/c003.png'> = ไม่ได้รับการตรวจฟัน "+rok+" คน<br> <img src='../img/c006.png'> = พบฟันผุ "+hok+" คน";		
...
function getXls(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datepicker-th1').value;			
		var url = "../xls/xls_epi_tooth.php?str="+str+"&village="+village;
	window.open(url,'data','top=120,left=250,width=700,height=550');
}
function getchart(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datepicker-th1').value;			
		var url = "../chart/chart_epi_tooth.php?str="+str+"&village="+village;
	window.open(url,'data','top=120,left=250,width=900,height=500');
}
function getsum(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datepicker-th1').value;
		var url = "../sum/sum_epi_tooth.php?str="+str+"&village="+village;
