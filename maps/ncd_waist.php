    ...
    <body class="skin-blue" onLoad="adjustWindow()">
		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				<form role="form">
        		<?php
				$txt = "<p align='center'><strong>รายงานประชาชนอายุ 15 ปีขึ้นไป <br>ที่ได้รับการวัดรอบเอว</strong></p>";
				...
				$txt .= "<div class='form-group'>
             				<label>เลือกกลุ่ม:</label>
                                ...
                                <select name='chk_ncd' id='chk_ncd' class='form-control' onchange='getData();'><option value='1'>แสดงทุกคน</option><option value='2'>เฉพาะผู้ที่ได้รับการวัด</option><option value='3'>เฉพาะผู้ที่รอบเอวเกิน</option><option value='4'>เฉพาะผู้ที่ยังไม่ได้วัด</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";
...
function getData(){
			...
			var url = "../genxml/genxml_ncd_waist.php?str="+str+"&village="+village+"&chk_ncd="+chk_ncd;
		  ...
		  var waist_chk = xmldata[i].getAttribute("waist_chk");
		  var screen_date = xmldata[i].getAttribute("screen_date");
		  var weight = xmldata[i].getAttribute("weight");
		  var height = xmldata[i].getAttribute("height");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+" อายุ "+age+" ปี<br><img src='../img/"+simg+"'>บ้านเลขที่ "+hono+" หมู่ " +vill +"<br> วันที่สำรวจ "+screen_date+" รอบเอว "+waist+" cm <br>"+waist_chk;
		   	html2 = pname+" อายุ "+age+" ปี<br>หมู่ "+moo+ " บ้านเลขที่ "+hono+"<br> วันที่สำรวจ "+screen_date+" รอบเอว "+waist+" cm <br>"+waist_chk;
			var icon;
			if(chk == "1"){
				hno = hno + 1;
			 	icon = customIcons['c1'];
			...
			var legent = "<img src='../img/c001.png'> = รอบเอวปกติ "+rok+" คน<br> <img src='../img/c003.png'> = รอบเอวเกิน "+hno+" คน<br> <img src='../img/c006.png'> = ยังไม่ได้วัดรอบเอว "+hok+" คน";		
...
function getXls(){
		...
		var chk_ncd = document.getElementById('chk_ncd').value;
		var str = document.getElementById('datestart').value;			
		var url = "../xls/xls_ncd_waist.php?str="+str+"&village="+village+"&chk_ncd="+chk_ncd;
	window.open(url,'data','top=120,left=250,width=700,height=550');
}
function getchart(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datestart').value;		
		var url = "../chart/chart_ncd_waist.php?str="+str+"&village="+village;
	window.open(url,'data','top=120,left=250,width=900,height=500');
}
function getsum(){
		var village = document.getElementById('village').value;
		var str = document.getElementById('datestart').value;			
		var url = "../sum/sum_ncd_waist.php?str="+str+"&village="+village;
