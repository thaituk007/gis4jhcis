		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="left-side sidebar-offcanvas">
				...
				$txt .= "<div class='form-group'>
             				<label>ประเภท Lab : </label>
                  				<div class='input-group'>
                   					<div class='input-group-addon'>
                     				<i class='fa fa-tasks'></i>
                      				</div>
                                <select name='app_type' id='app_type' class='form-control' onchange='getData();'><option value='pregtest'>ตรวจทดสอบการตั้งครรภ์</option></select>
                            	</div><!-- /.input group -->
                		</div><!-- /.form group -->";
				$txt .= "<div class='form-group'>
...
function getData(){
			...
			var url = "../genxml/genxml_lab_all.php?str="+str+"&village="+village+"&app_type="+app_type;
		  ...
		  var labresult = xmldata[i].getAttribute("labresult");
		  var userv = xmldata[i].getAttribute("userv");
		  ...
		  var latlng = new google.maps.LatLng(lat, lng);
			...
			html = pname+"<br><img src='../img/"+simg+"'> หมู่ "+moo+ " บ้านเลขที่ " + hno+" <br>วันที่ตรวจ "+sick+"<br> ผลการตรวจ"+labresult;
		   	html2 = pname+"<br>บ้านเลขที่ "+hno+" หมู่ " +moo+" <br>วันที่ตรวจ "+sick + "<br>ผลการตรวจ " + labresult;
...
function getXls(){
	...
	var app_type = document.getElementById("app_type").value;
	var str = document.getElementById('datestart').value;
	var url = "../xls/xls_lab_all.php?str="+str+"&village="+village+"&app_type="+app_type;
...
function getchart(){
	...
	var app_type = document.getElementById("app_type").value;
	var str = document.getElementById('datestart').value;
	var url = "../chart/chart_lab_all.php?str="+str+"&village="+village+"&app_type="+app_type;
	window.open(url,'data','top=120,left=250,width=800,height=500');
}
function getsum(){
	var village = document.getElementById('village').value;
	var app_type = document.getElementById("app_type").value;
	var str = document.getElementById('datestart').value;
	var url = "../sum/sum_lab_all.php?str="+str+"&village="+village+"&app_type="+app_type;
	window.open(url,'data','top=120,left=250,width=800,height=500');
}
</script>
